...
namespace VWeaponEditor.Core.Shortcuts.Inputs {
    /// <summary>
    ...
    public readonly struct KeyStroke : IInputStroke {
        ...
        /// </summary>
        public static Func<int, string> ReadableKeyCodeProvider { get; set; } = (x) => new StringBuilder(16).Append("KEY(").Append(x).Append(')').ToString();

        /// <summary>
        /// A non-null function for converting a keyboard modifier flag set into a string representation
        /// </summary>
        public static Func<int, bool, string> ReadableModifierProvider { get; set; } = (x, s) => new StringBuilder(16).Append("MOD(").Append(x).Append(')').ToString();
        ...
        public string ToString(bool appendIsReleaseOnly, bool useSpacers) {
            ...
            string mod = ReadableModifierProvider(this.Modifiers, useSpacers);
            if (mod.Length > 0) {
                sb.Append(mod).Append(useSpacers ? " + " : "+");
            }

            sb.Append(ReadableKeyCodeProvider(this.KeyCode));
