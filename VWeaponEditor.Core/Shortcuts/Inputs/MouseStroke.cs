...
namespace VWeaponEditor.Core.Shortcuts.Inputs {
    public readonly struct MouseStroke : IInputStroke {
        ...
        /// </summary>
        public static Func<int, string> ReadableMouseButtonProvider { get; set; } = (x) => new StringBuilder(20).Append("MOUSE(").Append(x).Append(')').ToString();
        ...
        public string ToString(bool appendClickCount, bool appendDelta, bool useSpacers) {
            ...
            string mod = KeyStroke.ReadableModifierProvider(this.Modifiers, useSpacers);
            if (mod.Length > 0) {
                sb.Append(mod).Append(useSpacers ? " + " : "+");
            }

            sb.Append(ReadableMouseButtonProvider(this.MouseButton));
